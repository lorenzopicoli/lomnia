  ...
  const minMax = data.reduce(
    (acc, curr) => {
      ...
      for (const key of keys) {
        ...
        acc.min[key] =
          isNumber(pathedValue) &&
          pathedValue < (acc.min[key].value ?? Number.MAX_SAFE_INTEGER)
            ? {
                value: pathedValue,
                entry: curr,
              }
        ...
        acc.max[key] =
          isNumber(pathedValue) &&
          pathedValue > (acc.max[key].value ?? Number.MAX_SAFE_INTEGER)
            ? {
                value: pathedValue,
                entry: curr,
              }
