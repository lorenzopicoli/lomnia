...
export function GenericChartSynchronized<T extends object>(props: {
  ...
  const { ref: tooltipRef, width: tooltipWidth, height: tooltipHeight } = useElementSize();
  ...
  }, [hashByChartId, memoizationContext, props.mainChart.data, props.mainChart.id, synchronizedContext?.currentDatum]);
  ...
  }, [hashByChartId, memoizationContext, props.secondaryCharts, synchronizedContext?.currentDatum]);

  const datumX = useMemo(() => nearestDatum?.x, [nearestDatum]);
  const datumY = useMemo(() => nearestDatum?.y, [nearestDatum]);

  const formattedX = useMemo(() => (isDateLike(datumX) ? format(datumX, "dd/MM HH:mm") : datumX), [datumX]);
  const formattedY = useMemo(() => Number(datumY?.toFixed(2)), [datumY]);

  const targetRadius = 5;
  const offsetX = useMemo(() => (isScaleBand(xScale) ? xScale.scale.bandwidth() / 2 : 0), [xScale]);
  ...
  const isTooLow = pointY + tooltipHeight > props.height - margin.top - margin.bottom;
  const isTooRight = pointX + tooltipWidth > props.width - margin.left - margin.right;
      ...
      >
        {(secondaryChartsDatums ?? []).map((chartDatum) => {
          ...
          const secondaryPointX =
            (xScale.scale(+chartDatum.datum.x) ?? 0) + (chartDatum.chart.type === ChartType.BarChart ? offsetX : 0);
