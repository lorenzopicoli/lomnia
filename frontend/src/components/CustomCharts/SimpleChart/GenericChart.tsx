 ...
 */
export function GenericChart<T extends object>(chart: InternalGenericChartProps<T>) {
  const {
    xScale,
    yScale,
    accessors: { getX, getY },
  ...
  const barGradient = () => (
    <LinearGradient id="gradient" from={theme.colors.violet[2]} to={theme.colors.violet[3]} rotate="-45" />
  );

  const areaStroke = " rgba(55, 57, 120, 1)";
  const areaGradient = () => <LinearGradient id="gradient-area" from="#D3A4FF" to="#9B9DDC" rotate="-45" />;

  const lineChartGradient = () => <LinearGradient id="gradient-line" from="#FFD700" to="#FF8C42" rotate="0" />;
      ...
      <>
        {chart.data.map((barData) => {
          ...
          return barHeight > 0 ? (
            <>
              {barGradient()}
              <path key={`bar-chart${barX}${barY}`} fill="url(#gradient)" d={rectPath} />
            </>
          ) : null;
        })}
      </>
