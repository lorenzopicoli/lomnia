...
function GenericChartAreaInternal<T extends object>(props: InternalGenericChartAreaProps<T>) {
  ...
  const orderedCharts = useMemo(() => {
    const ordered = secondaryCharts.concat(mainChart).sort((a, b) => order[a.type] - order[b.type]);
    return ordered;
  ...
  const getNearestDatum = useCallback(
    (svgPoint: ReturnType<typeof localPoint>) => {
      ...
      const bisect = bisector((v: T) => Number(mainChart.accessors.getX(v))).left;
  ...
  return (
    <svg height={height} width={width} overflow="visible">
      ...
      <GenericChartGrids xScale={xScale} yScale={yScale} width={width} height={height} margin={margin} />
      ...
      <GenericChartAxis xScale={xScale} yScale={yScale} height={height} margin={margin} mainChart={mainChart} />
      {/* Event capture rect */}
      <rect x={0} y={0} width={width} height={height} fill={"transparent"} {...eventEmitters} />
    </svg>
  );
}
