...
export type MemoizationContextType = {
  memoizedGetNearestDatum: (params: MemoizedGetNearestDatumParams) => MemoizedGetNearestDatumReturn;
};
export const MemoizationContext = createContext<MemoizationContextType | null>(null);

export const MemoizationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const memoizedGetNearestDatum = useCallback(() => {
    ...
    const func = (params: MemoizedGetNearestDatumParams): MemoizedGetNearestDatumReturn => {
      ...
      const bisect = bisector((v: { x: string | Date | number; y: number }) => Number(getX(v))).left;
  ...
  return <MemoizationContext.Provider value={values}>{children}</MemoizationContext.Provider>;
};
