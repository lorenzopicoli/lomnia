...
export function useChartData(
  ...
  const minYs = [habitData?.minY, weatherData?.minY, heartRateData?.minY].filter(isNotNill);
  const minXs = [habitData?.minX, weatherData?.minX, heartRateData?.minX].filter(isNotNill);
  const maxYs = [habitData?.maxY, weatherData?.maxY, heartRateData?.maxY].filter(isNotNill);
  const maxXs = [habitData?.maxX, weatherData?.maxX, heartRateData?.maxX].filter(isNotNill);

  const minY = Math.min(...minYs);
  const minX = minXs.length === 0 ? 0 : minXs.reduce((acc, curr) => (acc < curr ? acc : curr));
  const maxY = Math.max(...maxYs);
  const maxX = maxXs.length === 0 ? 0 : maxXs.reduce((acc, curr) => (acc > curr ? acc : curr));
  ...
  const mainChart: GenericChartProps<{ x: string | Date | number; y: number }> = {
    ...
    type: mainShape.type,
    data:
      habitData?.data[mainShape.yKey] ?? weatherData?.data[mainShape.yKey] ?? heartRateData?.data[mainShape.yKey] ?? [],
    minX,
    maxX,
    minY,
    maxY,
  ...
  const secondaryCharts: GenericChartProps<{
    ...
    type: shape.type,
    data: habitData?.data[shape.yKey] ?? weatherData?.data[shape.yKey] ?? heartRateData?.data[shape.yKey] ?? [],
    minX,
    maxX,
    minY,
    maxY,
