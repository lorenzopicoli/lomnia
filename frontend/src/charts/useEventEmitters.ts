// TAKEN FROM https://github.com/airbnb/visx/blob/master/packages/visx-xychart/src/hooks/useEventEmitters.ts
 ...
 */
export default function useEventEmitters(params: PointerEventEmitterParams) {
  const {
    source,
    chartId,
    getNearestDatum,
  ...
  const emit = useEventEmitter({ chartId, getNearestDatum });
  ...
  const emitPointerDown = useCallback((event: PointerEvent) => emit?.("pointerdown", event, source), [emit, source]);
