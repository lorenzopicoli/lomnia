...
import {
	type ExternalDnsQuery,
	externalDnsQueriesTable,
} from "./schema/tables";
	...
	private async importInternal(params: {
		...
		while (events?.length !== 0) {
			...
			if (events.length > 0) {
				...
				const timeElapsed = differenceInSeconds(new Date(), this.jobStart);
				const eta =
					(totalEstimate - importedCount) * (timeElapsed / importedCount);
				...
				console.log(
					`DNS Queries external import progress: ${importedCount}/${totalEstimate}. ETA: ${formatted}...`,
				);
			}
		}
