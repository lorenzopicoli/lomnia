...
export async function exportLocationsToJSON(
  ...
  while (true) {
    ...
    const locations = await db
      .select()
      .from(locationsTable)
      .leftJoin(locationDetailsTable, eq(locationsTable.locationDetailsId, locationDetailsTable.id))
      .limit(batchSize)
      .offset(offset);
    ...
    for (const row of locations) {
      ...
      const feature: GeoJSONFeature = {
        ...
        properties: {
          ...
          connection: location.connectionStatus === "data" ? "mobile" : location.connectionStatus || "wifi",
          vertical_accuracy: location.verticalAccuracy || 0,
          accuracy: location.accuracy || 0,
          timestamp: location.locationFix ? Math.floor(location.locationFix.getTime() / 1000) : 0,
          latitude: coords.lat,
          mode: null,
          inrids: null,
          in_regions: null,
...
async function writeToFile(features: GeoJSONFeature[], fileIndex: number, outputDir: string) {
