...
export async function getLocationsTimeline(params: { startDate: DateTime; endDate: DateTime }) {
    ...
    cte
      ...
      .where(
        sql`
            ...
            ${params.startDate ? sql`AND ${locationsTable.locationFix} >= ${params.startDate.toISO()}` : sql``}
            ${params.endDate ? sql`AND  ${locationsTable.locationFix} <= ${params.endDate.toISO()}` : sql``}
      `,
      ),
  );
  ...
  const baseActivitiesIslands = db.$with("activities_islands").as((cte) =>
    cte
      ...
      .groupBy(sql`${baseLocations.locationDetailsId}, (${baseLocations.totalSeq} - ${baseLocations.partitionSeq})`),
  );
  ...
  const durationIslands = db.$with("durations_islands").as((cte) =>
    cte
      ...
      .groupBy(sql`(${activitiesIslands.totalSeq} - ${activitiesIslands.partitionSeq})`)
